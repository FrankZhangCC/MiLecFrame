...
### 🟢 新功能：矩形三层效果栈 (feat, 90b637a)

- 每个自定义矩形（`layout.rectangles`）可独立开启纯色填充（`fill`）、高斯模糊磨砂底（`gaussian_blur`）、内描边（`stroke`），固定合成层级"模糊 → 填充 → 内描边"，内描边不改变矩形外部尺寸与定位
- 兼容分流：只有显式写出 `fill:` 字段或开启 `stroke`/`gaussian_blur` 的矩形才迁移到原图上方的效果栈；未使用新字段的全部内置样式输出像素级不变（18 张基线矩阵回归恒一致）
- 描边色支持独立明暗自适应键（`custom_{rect}_stroke_{dark/light}_color`），未配置时按四级回退链回退到填充色；厚度按原图短边比例换算，不足 1px 保底 1px、达短边一半自动截断并 warning
- 三层独立校验与降级；照片内矩形按原图坐标精确取样模糊，跨界/照片外矩形延续背景"原图拉伸到画布"模糊语义，完全在画布外的矩形零计算；`opacity>=1` 填充完全覆盖时跳过模糊
- 样式编辑器加载→保存不再丢失 `rectangles` 与 `custom_rect_*` 颜色等未识别字段（透传容器）
- D2 零配置升级：FrameBar 类样式只需在 `rect_01` 追加三块效果字段即可升级磨砂信息条，`info_position` 零改动，文字自动叠在磨砂条上

### 🟢 性能：全图高斯统一短路 + 两级模糊缓存 (feat, 90b637a)

- 全图高斯需求统一判定（`blur plan` DEBUG 日志）：高斯背景+无画布扩展+无圆角场景不再全量模糊（33MP 样片 2.64s → 0.10s，约 26 倍）
- 一级缓存（每帧）：背景与多个矩形共享同源同半径卷积，多矩形不同尺寸派生零重复卷积
- 二级缓存（跨帧，GUI 交互）：`PreparedBlurLRU` 按 64 MiB 预算 LRU 缓存工作分辨率卷积结果；主处理页与样式编辑器接入页面级缓存，切换背景颜色/透明度/饱和度即时生效不再重新卷积；缓存键含源内容摘要+尺寸+模式+预处理版本+半径+算法版本，跨图/跨预处理不误命中；批量处理不接二级缓存，内存平稳
- 高斯算法拆分为 prepare/render_prepared/color_overlay 三阶段，全程 float32 接力，背景输出逐像素不变

### 🟢 新功能：旋转适配——用户可选顺/逆时针整帧旋转 (feat, 555f9cc)

- 新增 `orientation_adaptation` 工具模块：样式默认值校验、用户/样式优先级解析、无损 90° 离散转置、高斯二级缓存键方向后缀派生
- 渲染管线整帧前置旋转/输出还原：用户显式选择顺/逆时针对所有图片（横图/竖图/方形图）生效；样式预设仅对竖图生效；未声明样式在默认模式下输出与历史版本逐像素一致
- 图片处理页"旋转适配"四选项菜单（默认跟随样式/不旋转/顺时针/逆时针），动态提示、稳定内部值持久化；三个下拉框统一宽度对齐
- 样式可声明 `default_portrait_adaptation` 竖图默认方向（非法值拒绝加载整份样式）；样式编辑器支持该字段声明与 YAML 往返；FilmClip 两个变体预设 clockwise

### 🔴 修复：信息栏格式显示"未知" (fix, c17f639)

- EXIF 转正副本不继承 `format` 属性导致信息栏格式显示"未知"，转正后回填修复

### 🔧 重构 (refactor, 90b637a)

- `render_frame` 签名从 13 参数收敛为 4 参数、`ImageProcessor.process` 从 15 参数收敛为 6 参数（`RenderMetadata`/`RenderOptions` dataclass 打包，零行为变化，CLI/GUI 全调用点迁移，像素对比验证一致）
- plan 类文档归档至 `docs/plans/`；样式模板/样式指南/开发文档同步更新；AGENTS.md 增补合入门禁与版本号赋予时机规则
