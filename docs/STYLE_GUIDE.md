...
### 2.4 竖图方向适配默认值（default_portrait_adaptation）

可选的**顶层**字段，声明本样式对竖拍照片的建议处理方向：

```yaml
name: 我的样式
default_portrait_adaptation: clockwise   # clockwise / counterclockwise / none
```

- **语义**：竖图（EXIF 转正后 height > width）在渲染前整帧顺/逆时针旋转 90°，按横向构图完成全部布局（背景、画布、矩形、文字、Logo、水印），输出前再旋转还原。适合"信息条在长边一侧"的横向排版样式——竖图直接渲染时信息条会挤在窄边上，声明适配后信息条落在照片长边一侧，效果与横图一致。
- **作用范围**：该预设**仅对竖图生效**——横图与方形图即使样式声明了适配也保持原方向（预设是样式对竖拍构图的建议）。用户在图片处理页**显式选择**"顺时针适配 / 逆时针适配"时不受此限，对所有图片生效。
- **缺省行为**：字段缺失等同于 `none`（不适配），**所有未声明的现有样式行为不变**。
- **与用户选择的关系**：仅当用户在图片处理页"旋转适配"菜单选择 **默认（跟随样式）** 时本字段生效；用户显式选择"不旋转 / 顺时针适配 / 逆时针适配"时**覆盖**本值。
- **合法值**：仅 `clockwise` / `counterclockwise` / `none`。布尔值、`null`、大小写变体、未知字符串会被 StyleManager 拒绝加载（错误日志点名该字段）。

> ⚠ **变体必须逐个声明**：文件夹样式的每个变体都是独立完整配置，字段不会从 `default.yaml` 继承。若只给 `default.yaml` 声明，当 `custom_text` / `location` 等缺失触发变体切换后，适配会"消失"。需要适配的样式，请在**每个**变体文件中都写入相同值（参考"胶片夹风格 FilmClip"的两个变体）。
...
│  └──────────────────┘      │
│  ┌──────────────────┐      │
│  │ ★ 效果栈矩形层 ★  │      │  ← 显式配置效果字段的 rectangles 在此
│  └──────────────────┘      │
│      文字 / Logo 层        │
└────────────────────────────┘
```
...
每个矩形可以独立开启三个效果层，固定合成层级为：

```text
高斯模糊底层 → 纯色填充中层 → 内描边顶层
```

**⚠ 效果栈迁移提示**：只要显式写出 `fill:` 字段（哪怕 `enabled: false`），
或开启 `stroke.enabled: true` / `gaussian_blur.enabled: true`，该矩形就会
从"原图下方"迁移到"原图上方"的效果栈，从而在照片内部可见。
仅写 `stroke.enabled: false` 或 `gaussian_blur.enabled: false` 不会触发迁移。

| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `fill.enabled` | `bool` | `true` | 是否绘制纯色填充；缺失时保持旧样式默认开启语义 |
| `gaussian_blur.enabled` | `bool` | `false` | 是否绘制矩形底层的磨砂模糊 |
| `gaussian_blur.radius` | `int` | `200` | 全分辨率等效模糊半径（算法内部按工作分辨率同比缩放），上限 1000 |
| `stroke.enabled` | `bool` | `false` | 是否绘制顶部内描边 |
| `stroke.radius_ratio` | `float` | `0.005` | 描边厚度相对原图参照边（短边）的比例；**内描边不改变矩形外部尺寸与定位** |
| `stroke.opacity` | `float` | `1.0` | 描边独立透明度，不复用填充 `opacity` |

字段语义说明：

- `opacity` 继续保持原有语义：只控制**填充层**透明度，不影响模糊与描边。
- `opacity: 1.0` 的有效填充会完全覆盖模糊层——此时模糊层自动跳过计算（视觉结果相同，速度更快）。
- 描边采用**内描边**：`width_ratio` / `height_ratio` / 定位结果描述的始终是矩形外轮廓，开启描边不会扩大矩形或额外越界；描边厚度换算后不足 1px 时保底 1px，达到矩形短边一半时自动截断（保留至少 1px 内容区）并记录 warning。
- 描边颜色使用独立的明暗自适应键（可选）：`custom_{rect_name}_stroke_dark_color` / `custom_{rect_name}_stroke_light_color`。未配置时按"当前方案描边色 → 另一方案描边色 → 当前方案填充色 → 另一方案填充色"四级回退；整条回退链均无效时只跳过描边层，不影响模糊或填充。
- 三层各自独立校验与降级：某一层无效不影响其余层；只有三层全部关闭或无效时才跳过整个矩形。
- 高斯模糊矩形与背景共享同一张卷积缓存：同源同半径只计算一次；照片内矩形按原图坐标精确取样。

**跨界/照片外矩形的模糊语义**：矩形跨越照片边界或完全位于照片外时，
模糊源延续现有背景的"原图拉伸到画布"语义（先模糊原图、再缩放到完整
...
layout:
  rectangles:
    rect_01:
      ...
      # ── 追加这三块即进入效果栈（原图上方，照片内可见） ──
      fill:                     # 显式写出即选择效果栈语义
        enabled: true
      gaussian_blur:            # 底层磨砂
        enabled: true
        radius: 200             # 与背景默认半径一致利于共享卷积
      stroke:                   # 顶部内描边
        enabled: true
        radius_ratio: 0.002
        opacity: 0.60
